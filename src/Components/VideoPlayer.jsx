...
const VideoPlayer = ({ src, type, controls = false, autoplay = true, loop = true, muted = true }) => {
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (videoElement) {
                const offset = window.scrollY * 0.5; // Adjust for desired parallax effect strength
    ...
    return (
        <div className="relative h-[60vh] md:h-[80vh] lg:h-[100vh] overflow-hidden">
            <video
                className="absolute top-0 left-0 w-full h-full object-cover parallax-video"
                controls={controls}
                autoPlay={autoplay}
                loop={loop}
                muted={muted}
